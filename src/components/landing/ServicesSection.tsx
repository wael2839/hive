        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {t.items.map((item, i) => (
            <ScrollReveal key={item.title} delayMs={60 + i * 40}>
              <article className="service-card group relative overflow-hidden rounded-2xl border border-hive-border bg-[var(--hive-card-glass)] p-7">
                <div
                  className="pointer-events-none absolute -right-10 -top-10 h-40 w-40 rounded-full bg-[radial-gradient(circle_at_center,rgba(255,232,140,0.14),transparent_65%)] opacity-0 transition-opacity duration-500 group-hover:opacity-100"
                  aria-hidden
                />
                <div className="mb-5 inline-flex h-11 w-11 items-center justify-center rounded-xl border border-hive-gold/25 bg-gradient-to-br from-hive-gold/25 to-transparent text-sm font-bold text-hive-gold-light">
                  <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-hive-black/40 text-[11px]">
